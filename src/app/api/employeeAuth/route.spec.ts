// src/app/api/employeeAuth/route.spec.ts
...
describe("employeeAuth POST API", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("should return 300 if user is not verified", async () => {
    (db.where as jest.Mock).mockResolvedValueOnce([{ role: "employee", status: "pending" }]);
  ...
  it("should return 200 and role if user is an employee and verified", async () => {
    ...
    expect(responseData).toEqual("employee");
  });
